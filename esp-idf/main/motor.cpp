...
// Fault-recovery path — only called on stalls. Pulses nSLEEP to clear latched DRV8313
// OCP/TSD, zeros PID state, re-enables.
...
void motor_idle_anchor() {
    ...
    if (!g_motor.enabled) {
        ...
        g_target     = g_motor.shaft_angle;
        g_target_vel = 0.0f;
    }
}

// Trapezoidal velocity profile — matches Arduino M3b's rampTarget exactly.
static void ramp_target() {
    ...
    float cmd = g_commanded.load();
    float err = cmd - g_target;
    float decel_dist = (g_target_vel * g_target_vel) / (2.0f * g_accel);
    ...
    g_target += g_target_vel * dt;

    if ((err > 0 && g_target >= cmd) || (err < 0 && g_target <= cmd)) {
        g_target     = cmd;
        g_target_vel = 0.0f;
    }
}
...
void motor_tick() {
    ...
    float cmd = g_commanded.load();
    bool ramping     = fabsf(cmd - g_target) > 1e-4f || fabsf(g_target_vel) > 1e-4f;
    bool pos_err_big = g_motor.enabled && fabsf(g_target - g_motor.shaft_angle) > IDLE_POS_THRESH;
    ...
    // M3b pattern: loopFOC + move sequential in the task, no decoupled ISR.
    g_motor.loopFOC();
    g_motor.move(g_target);
