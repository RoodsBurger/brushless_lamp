...
// PATCH: 20 kHz is right at the upper limit of human hearing — Arduino on the same C6
// runs at 25 kHz via arduino-esp32's bundled SimpleFOC and is noticeably quieter. The
// upstream esp_simplefoc driver hardcodes the MCPWM `period_ticks` from this macro and
// ignores BLDCDriver3PWM::pwm_frequency, so a one-line change here is the cleanest fix.
#define _PWM_FREQUENCY 25000                           /*!< default frequency of MCPWM (was 20000 — patched for inaudibility) */
...
#define _LEDC_FREQUENCY (25 * 1000)                    /*!< Frequency of LEDC (matches MCPWM patch above) */
